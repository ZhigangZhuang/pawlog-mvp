...
export function TimelineList({
  ...
  if (items.length === 0) {
    return (
      <div className="rounded-lg bg-white p-5 text-center text-sm text-stone-500 ring-1 ring-sand/70">
        {emptyText}
      </div>
    );
  }
    ...
    <div className="space-y-3">
      {items.map((item) => {
        ...
        return (
          <div
            key={item.id}
            className="flex gap-3 rounded-lg bg-white p-4 ring-1 ring-sand/70"
          >
            ...
            <div className="min-w-0 flex-1">
              ...
              <p className="mt-2 text-xs text-stone-400">
                ...
                {item.source
                  ? ` · 来源：${sourceLabel(item.source.source_type)}`
                  : ""}
              </p>
            </div>
          </div>
