...
export function InboxPhotosPage({
  state,
  onBack,
  onSave,
  onCreateAnimal,
  ...
  const inbox = state.inboxPhotos.filter(
    (photo) => photo.status === "unassigned",
  );
  ...
  const assign = (photoId: string) => {
    ...
    const nextState = {
      ...state,
      inboxPhotos: state.inboxPhotos.map((item) =>
        item.id === photoId
          ? {
              ...item,
              status: "assigned" as const,
              assigned_animal_id: targetAnimalId,
            }
          : item,
      ),
      photos: [
    ...
    onSave(
      withChangeLog(nextState, {
        animal_id: targetAnimalId,
        action: "assigned_inbox_photo",
        after: { inbox_photo_id: photoId },
      }),
    );
  };
  ...
  const ignore = (photoId: string) => {
    onSave({
      ...state,
      inboxPhotos: state.inboxPhotos.map((item) =>
        item.id === photoId ? { ...item, status: "ignored" } : item,
      ),
    });
  };
    ...
    >
      <div className="space-y-5">
        <div className="rounded-lg border border-green-200 bg-green-50 p-4 text-sm leading-6 text-green-900">
          MVP 暂不做 AI
          识别。你可以先批量导入照片，再手动归档到已有动物，或用照片新建档案。
        </div>
        ...
        <div className="space-y-3 rounded-lg bg-white p-4 ring-1 ring-sand/70">
          ...
          <div className="flex gap-2">
            <input
              className="min-w-0 flex-1 rounded-lg border border-sand px-3 py-2 outline-none focus:border-clay"
              value={imageUrl}
              onChange={(event) => setImageUrl(event.target.value)}
              placeholder="粘贴图片地址"
            />
            <button
              className="rounded-full bg-clay px-4 text-white"
              onClick={addPhoto}
            >
              <Plus size={18} />
            </button>
        ...
        <div className="rounded-lg bg-white p-4 ring-1 ring-sand/70">
          ...
          <select
            ...
            onChange={(event) => setTargetAnimalId(event.target.value)}
          >
            {state.animals
              .filter((animal) => animal.archive_status === "active")
              .map((animal) => (
        ...
        <div className="space-y-3">
          {inbox.map((photo) => (
            <div
              key={photo.id}
              className="overflow-hidden rounded-[20px] bg-white ring-1 ring-sand/70"
            >
              <img
                className="h-56 w-full object-cover"
                src={photo.image_url}
                alt={photo.note || "待归档照片"}
              />
              <div className="space-y-3 p-4">
                ...
                <div className="grid grid-cols-3 gap-2">
                  <button
                    className="rounded-full bg-clay px-3 py-2 text-sm font-bold text-white"
                    onClick={() => assign(photo.id)}
                  >
                    <Check className="inline" size={15} /> 归档
                  </button>
                  <button
                    className="rounded-full bg-white px-3 py-2 text-sm font-bold text-ink ring-1 ring-sand/80"
                    onClick={onCreateAnimal}
                  >
                    <Camera className="inline" size={15} /> 新建
                  </button>
                  <button
                    className="rounded-full bg-stone-100 px-3 py-2 text-sm font-bold text-stone-600"
                    onClick={() => ignore(photo.id)}
                  >
