...
export function AnimalDetailPage({
  ...
  const posts = [...state.feedRecords]
    .filter((item) => animalIdsForRecord(item).includes(animal.id))
    .sort(
      (a, b) =>
  ...
  const locations = state.locations.filter(
    (item) => item.animal_id === animal.id,
  );
  const logs = state.changeLogs
    .filter((log) => log.animal_id === animal.id)
    .slice(0, 6);
  const tagById = new Map(state.tags.map((tag) => [tag.id, tag.name]));
  const animalById = new Map(state.animals.map((item) => [item.id, item]));
  const relationships = state.animalRelationships.filter(
    (item) =>
      item.from_animal_id === animal.id || item.to_animal_id === animal.id,
  );
  const families = state.animalFamilies.filter((family) =>
    family.member_animal_ids.includes(animal.id),
  );
  const transfers = state.animalTransfers.filter(
    (transfer) => transfer.animal_id === animal.id,
  );
  const relatedAnimals = relationships
    .map((relation) => {
      const otherId =
        relation.from_animal_id === animal.id
          ? relation.to_animal_id
          : relation.from_animal_id;
      const other = animalById.get(otherId);
      if (!other) return null;
      return {
        animal: other,
        relation,
        outgoing: relation.from_animal_id === animal.id,
      };
    })
    .filter(Boolean) as Array<{
    animal: Animal;
    relation: (typeof relationships)[number];
    outgoing: boolean;
  }>;
    ...
    >
      <div className="space-y-4 pb-24">
        <section className="rounded-[22px] bg-white px-4 py-5 text-center ring-1 ring-sand/70">
          <img
            className="mx-auto h-24 w-24 rounded-full object-cover ring-4 ring-cream"
            src={animal.cover_image_url}
            alt={animal.name}
          />
          <h1 className="mt-3 text-2xl font-black">{animal.name}</h1>
          <p className="mt-1 text-sm text-stone-500">
            {animal.color || speciesLabels[animal.species]}
            {isStray
              ? `流浪${speciesLabels[animal.species]}`
              : animal.breed
                ? ` · ${animal.breed}`
          ...
          <p className="mt-1 text-xs text-stone-400">
            {firstLocationText(locations, isStray)} · {posts.length} 条动态 ·{" "}
            {photos.length} 张照片
          </p>
          <div className="mt-3 flex flex-wrap justify-center gap-1.5">
            <TinyTag>
              {isStray ? `流浪${speciesLabels[animal.species]}` : "自家宠物"}
            </TinyTag>
            {animal.health_status ? (
              <TinyTag>{healthLabels[animal.health_status]}</TinyTag>
            ) : null}
            {isStray && animal.rescue_status ? (
              <TinyTag>{rescueLabels[animal.rescue_status]}</TinyTag>
            ) : null}
            {permissionLabels(animal).map((tag) => (
        ...
        {isStray ? (
          <div className="flex gap-2 rounded-2xl border border-green-200 bg-green-50 p-3 text-xs leading-5 text-green-900">
            ...
            <p>
              流浪动物分享时会隐藏精确位置、固定出没时间、幼崽窝点和抓捕计划。
            </p>
          </div>
        ) : null}
          ...
          <section className="space-y-3 rounded-[20px] bg-white p-4 ring-1 ring-sand/70">
                  ...
                  {families[0].member_animal_ids
                    ...
                    .map((member) => (
                      <button
                        ...
                        onClick={() =>
                          member.id !== animal.id && onOpenAnimal(member.id)
                        }
                      >
                        <img
                          className={`mx-auto h-11 w-11 rounded-full object-cover ${member.id === animal.id ? "ring-2 ring-clay" : ""}`}
                          src={member.cover_image_url}
                          alt={member.name}
                        />
                        <span className="mt-1 block truncate text-xs font-bold text-stone-600">
                          {member.id === animal.id ? "当前" : member.name}
                        </span>
                      </button>
                    ))}
                </div>
            ...
            {relatedAnimals.length ? (
              <div className="flex gap-2 overflow-x-auto pb-1">
                {relatedAnimals
                  .slice(0, 5)
                  .map(({ animal: related, relation, outgoing }) => (
                    <button
                      key={relation.id}
                      className="shrink-0 rounded-full bg-cream px-3 py-2 text-left text-xs font-semibold text-stone-600"
                      onClick={() => onOpenAnimal(related.id)}
                    >
                      {related.name} ·{" "}
                      {relationLabel(relation.relation_type, outgoing)}
                    </button>
                  ))}
              </div>
            ) : null}
        ...
        <div className="flex justify-around border-b border-sand/80 text-sm font-bold text-stone-500">
          {[
            ...
            <button
              key={value}
              className={`border-b-2 px-3 py-2 ${tab === value ? "border-clay text-clay" : "border-transparent"}`}
        ...
        {tab === "timeline" ? (
          <div className="space-y-4">
            {posts.length ? (
              ...
              <div className="rounded-2xl bg-white p-5 text-sm text-stone-500 ring-1 ring-sand/70">
                还没有动态。
              </div>
            )}
          </div>
          ...
          <div className="space-y-3">
            {locations.length ? (
              locations.map((location) => (
                <div
                  key={location.id}
                  className="flex items-start gap-3 rounded-2xl bg-white p-4 ring-1 ring-sand/70"
                >
        ...
        {tab === "profile" ? (
          <div className="space-y-3">
            <details
              className="rounded-2xl bg-white p-4 ring-1 ring-sand/70"
              open
            >
              ...
              <div className="mt-3 grid grid-cols-2 gap-x-4 gap-y-3 text-sm">
                ...
                <Info
                  label="年龄"
                  value={getAgeText(animal.birthday, animal.age_stage)}
                />
                <Info label="毛色" value={animal.color || "未记录"} />
                {!isStray ? (
                  <Info label="生日" value={formatShortDate(animal.birthday)} />
                ) : null}
                {!isStray ? (
                  <Info
                    label="到家日"
                    value={formatShortDate(animal.home_date)}
                  />
                ) : null}
              </div>
            </details>
            ...
            <details className="rounded-2xl bg-white p-4 ring-1 ring-sand/70">
              ...
              <p className="mt-3 text-sm text-stone-600">
                {permissionLabels(animal)
                  .map((tag) => `#${tag}`)
                  .join(" ")}
              </p>
              {animal.ownership_status === "transferred_out" ? (
                <p className="mt-2 text-sm leading-6 text-stone-600">
                  {animal.transfer_note ||
                    `现在由${animal.current_keeper_label || "朋友"}记录。`}
                </p>
              ) : null}
              {transfers.map((transfer) => (
                ...
                >
                  已转交给{transfer.to_user_label || "朋友"} ·{" "}
                  {transfer.keep_record_permission
                    ? "我仍可补充动态"
                    : "我保留查看成长动态"}
                </p>
              ))}
                    ...
                    <div className="mt-3 flex flex-wrap gap-2">
                      {family.member_animal_ids
                        ...
                        .map((member) => (
                          <button
                            key={member.id}
                            className="inline-flex items-center gap-2 rounded-full bg-white px-2 py-1 text-xs font-bold text-stone-600 ring-1 ring-sand/70"
                            onClick={() =>
                              member.id !== animal.id && onOpenAnimal(member.id)
                            }
                          >
            ...
            <details className="rounded-2xl bg-white p-4 ring-1 ring-sand/70">
              ...
              <div className="mt-3 space-y-2">
                {logs.length ? (
                  ...
                  <p className="text-sm text-stone-500">暂无变更历史。</p>
                )}
              </div>
            </details>
          </div>
        ...
        {canPost ? (
          <button
            className="fixed bottom-24 right-[max(1rem,calc((100vw-28rem)/2+1rem))] z-20 grid h-12 w-12 place-items-center rounded-full bg-clay text-white shadow-soft"
            onClick={() => onAddRecord()}
            aria-label="发动态"
          >
            <Plus size={24} />
      ...
      <div className="px-4 pt-4">
        ...
        {post.location_text ? (
          <p className="mt-0.5 text-xs text-stone-400">
            {animal.animal_origin === "stray" ? "模糊地点" : post.location_text}
          </p>
        ) : null}
      </div>
...
function permissionLabels(animal: Animal) {
  ...
  if (
    ...
    animal.visibility === "shared_readonly"
  )
    labels.push("只读");
...
function transferText(
  animal: Animal,
  transfer?: AppState["animalTransfers"][number],
) {
  if (animal.ownership_status === "transferred_out") {
    const keeper =
      animal.current_keeper_label || transfer?.to_user_label || "朋友";
    if (transfer?.keep_record_permission)
      return `现在住在${keeper}家，你仍可以补充旧照片和动态。`;
    return (
      animal.transfer_note ||
      `现在由${keeper}记录，你可以继续看到它的成长动态。`
    );
  }
...
function firstLocationText(
  locations: Array<{ location_name: string; is_sensitive: boolean }>,
  isStray: boolean,
) {
