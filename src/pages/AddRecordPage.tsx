...
export function AddRecordPage({ animal, state, initialType, groupId, onBack, onSave }: AddRecordPageProps) {
  ...
  const save = () => {
    if (!selectedAnimalIds.length) return;
    const now = new Date().toISOString();
    const source = groupId ? { source_type: "group_member" as const, source_user_id: "user_1", group_id: groupId, created_at: now } : { source_type: "self" as const };
    ...
    const nextState: AppState = {
      ...
      animals: state.animals.map((item) => {
        ...
        if (selectedAnimalIds.includes(item.id)) return upsertAnimalUpdatedAt(item);
        return item;
      }),
      postAnimals: [
      ...
      feedRecords: [
        {
          ...
          tag_ids: [],
          created_by: "user_1",
          source: groupId ? "shared_user" : "me",
          visibility: primaryAnimal.visibility === "public_card" ? "public_card" : groupId ? "shared" : "private",
          created_at: now,
        },
        ...state.feedRecords,
      ],
      ...
      locations:
        locationName
          ? [
              ...selectedAnimalIds.map((animalId): StrayLocation => {
                ...
                const precisionLevel = targetAnimal.animal_origin === "stray" && isSensitive ? "text_only" : isSensitive ? "exact" : "blurred";
                return {
                id: createId("loc"),
                animal_id: animalId,
                ...
                address_text: targetAnimal.animal_origin === "stray" ? "位置已模糊" : locationName,
        ...
        <div className="space-y-3">
          ...
          <div className="flex flex-wrap gap-2">
            {allowedTypes.map((item) => (
              <button key={item} className={`rounded-full px-3 py-1.5 text-sm font-semibold ${type === item ? "bg-clay text-white" : "bg-white text-stone-600 ring-1 ring-sand/70"}`} onClick={() => setType(item)}>
        ...
        <div className="rounded-2xl bg-white p-4 ring-1 ring-sand/70">
          ...
          {locationName ? (
            <label className="mt-3 flex items-center gap-2 text-sm text-stone-600">
              ...
              {containsStray ? "流浪动物地点默认模糊" : "仅自己可见"}
            </label>
          ) : null}
        </div>
