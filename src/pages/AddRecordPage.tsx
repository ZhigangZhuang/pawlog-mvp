import { Camera, ChevronDown, MapPin, Search, ShieldCheck, X } from "lucide-react";
...
import { activeAnimals, animalIdsForRecord, createId, upsertAnimalUpdatedAt } from "../utils/storage";
...
export function AddRecordPage({ animal, state, initialType, onBack, onSave }: AddRecordPageProps) {
  ...
  const { recentAnimals, frequentAnimals } = useMemo(() => {
    ...
    const sorted = [...availableAnimals].sort((a, b) => (lastSeen.get(b.id) || 0) - (lastSeen.get(a.id) || 0));
  ...
  const toggleAnimal = (id: string) => {
    const nextIds = selectedAnimalIds.includes(id) ? selectedAnimalIds.filter((x) => x !== id) : [...selectedAnimalIds, id];
  ...
  return (
    <AppShell title="发动态" subtitle={selectedAnimals.map((item) => item.name).join("、")} canGoBack onBack={onBack}>
      <div className="space-y-4 pb-28">
        ...
        {type === "health" && (
          <CompactSelect label="健康状态" value={healthStatus} onChange={(v) => setHealthStatus(v as HealthStatus)} options={healthLabels} />
        )}
        {type === "neuter_status" && (
          <CompactSelect label="绝育/TNR 状态" value={neuterStatus} onChange={(v) => setNeuterStatus(v as NeuterStatus)} options={neuterLabels} />
        )}
        {type === "rescue_status" && (
          <CompactSelect label="救助状态" value={rescueStatus} onChange={(v) => setRescueStatus(v as RescueStatus)} options={rescueLabels} />
        )}
        {type === "weight" && (
        ...
        <div className="rounded-2xl bg-white p-4 ring-1 ring-sand/70">
          <label className="block">
            ...
            <input
              ...
              placeholder={containsStray ? "例如：北门附近" : "例如：家里 / 公园"}
            />
          </label>
          {locationName && (
...
function AnimalPickerList({
  ...
  if (!animals.length) return <p className="text-sm text-stone-400">没有找到毛孩</p>;
  return (
            ...
            {selected && (
              <button
                ...
                className={`shrink-0 rounded-full px-2 py-0.5 text-xs font-bold ${
                  primaryId === item.id ? "bg-clay text-white" : "bg-white text-stone-500 ring-1 ring-sand/70"
                }`}
                onClick={(e) => {
                  e.preventDefault();
...
function CompactInput({ label, value, onChange, placeholder }: { label: string; value: string; onChange: (value: string) => void; placeholder: string }) {
  return (
    <label className="block rounded-2xl bg-white p-4 ring-1 ring-sand/70">
      ...
      <input className="w-full rounded-2xl border border-sand px-3 py-2 outline-none focus:border-clay" value={value} onChange={(e) => onChange(e.target.value)} placeholder={placeholder} />
    </label>
  );
}
...
function CompactSelect({ label, value, onChange, options }: { label: string; value: string; onChange: (value: string) => void; options: Record<string, string> }) {
  return (
    <label className="block rounded-2xl bg-white p-4 ring-1 ring-sand/70">
      <span className="mb-2 block text-sm font-bold text-stone-600">{label}</span>
      <select className="w-full rounded-2xl border border-sand bg-white px-3 py-2 outline-none focus:border-clay" value={value} onChange={(e) => onChange(e.target.value)}>
