...
export function CatalogPage({
  ...
  const shown = animals
    ...
    .filter((animal) =>
      `${animal.name}${animal.color || ""}${animal.features || ""}${animal.personality || ""}`.includes(
        query,
      ),
    )
    .sort(
      (a, b) =>
        animalPriority(a) - animalPriority(b) ||
        new Date(b.updated_at).getTime() - new Date(a.updated_at).getTime(),
    );
  const strayCount = animals.filter(
    (animal) => animal.animal_origin === "stray",
  ).length;
  const watchingCount = animals.filter(
    (animal) =>
      animal.animal_origin === "stray" &&
      animal.health_status &&
      animal.health_status !== "normal",
  ).length;
  const tnrCount = animals.filter(
    (animal) =>
      animal.animal_origin === "stray" &&
      animal.neuter_status !== "confirmed_neutered",
  ).length;
  const adoptableCount = animals.filter(
    (animal) =>
      animal.animal_origin === "stray" &&
      ...
      <div className="space-y-4 pb-24">
        <div className="flex items-center gap-2 rounded-full bg-white px-4 py-3 text-sm text-stone-500 ring-1 ring-sand/80">
          ...
          <input
            ...
            placeholder="搜索昵称、毛色、特征"
          />
        </div>

        <p className="text-sm font-semibold text-stone-500">
          已记录 {strayCount} 只流浪毛孩 · 待观察 {watchingCount} · 待绝育{" "}
          {tnrCount} · 可领养 {adoptableCount}
        </p>
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex rounded-full bg-white p-1 ring-1 ring-sand/70">
            <button
              className={`grid h-8 w-8 place-items-center rounded-full ${view === "grid" ? "bg-orange-50 text-clay" : "text-stone-500"}`}
            ...
            <button
              className={`grid h-8 w-8 place-items-center rounded-full ${view === "list" ? "bg-orange-50 text-clay" : "text-stone-500"}`}
          ...
          <div className="grid grid-cols-2 gap-3">
            {shown.map((animal) => (
              <AnimalGridCard
                ...
                animal={animal}
                postCount={
                  state.feedRecords.filter((record) =>
                    animalIdsForRecord(record).includes(animal.id),
                  ).length
                }
...
function AnimalGridCard({
  ...
  return (
    <button
      className="overflow-hidden rounded-[18px] bg-white text-left ring-1 ring-sand/70"
      onClick={onOpen}
    >
      <img
      ...
      <div className="space-y-2 p-3">
        <div>
          ...
          <p className="truncate text-xs text-stone-500">
            {postCount} 动态 · {animal.color || speciesLabels[animal.species]}
          </p>
        </div>
        <div className="flex flex-wrap gap-1.5">
          {catalogTags(animal)
            ...
            .map((tag) => (
              <span
                key={tag}
                className="rounded-full bg-orange-50 px-2 py-0.5 text-[11px] font-semibold text-clay"
              >
                #{tag}
              </span>
            ))}
...
function AnimalListCard({
  ...
  return (
    <button
      className="flex w-full items-center gap-3 rounded-lg bg-white p-3 text-left ring-1 ring-sand/70"
      onClick={onOpen}
    >
      <img
      ...
      <span className="min-w-0 flex-1">
        ...
        <span className="line-clamp-1 text-sm text-stone-500">
          {postCount} 动态 ·{" "}
          {catalogTags(animal)
            .map((tag) => `#${tag}`)
            .join(" ")}
...
function matchFilter(animal: Animal, filter: Filter, state: AppState) {
  ...
  if (filter === "stray_cat")
    return animal.animal_origin === "stray" && animal.species === "cat";
  if (filter === "stray_dog")
    return animal.animal_origin === "stray" && animal.species === "dog";
  if (filter === "watching")
    return (
      animal.animal_origin === "stray" &&
      Boolean(animal.health_status && animal.health_status !== "normal")
    );
  if (filter === "neutered")
    return (
      animal.animal_origin === "stray" &&
  ...
  if (filter === "not_neutered")
    return (
      animal.animal_origin === "stray" &&
      animal.neuter_status !== "confirmed_neutered"
    );
  if (filter === "rescue_needed")
    return (
      animal.animal_origin === "stray" && animal.rescue_status === "needs_help"
    );
  if (filter === "adoptable")
    return (
      animal.animal_origin === "stray" && animal.adoption_status === "available"
    );
  ...
  if (filter === "transferred")
    return (
      ...
      animal.ownership_status === "transferred_to_me"
    );
  if (filter === "family")
    return state.animalRelationships.some(
      (item) =>
...
function catalogTags(animal: Animal) {
  ...
  if (
    ...
    tags.push("只读");
  if (animal.animal_origin === "stray" && animal.species === "cat")
    tags.push("流浪猫");
  if (animal.animal_origin === "stray" && animal.species === "dog")
    tags.push("流浪狗");
...
function animalPriority(animal: Animal) {
  ...
  if (
    animal.rescue_status === "needs_help" ||
