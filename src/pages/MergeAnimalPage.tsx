...
export function MergeAnimalPage({
  state,
  animal,
  onBack,
  onSave,
}: MergeAnimalPageProps) {
  const candidates = activeAnimals(state).filter(
    (item) =>
      item.id !== animal.id && item.animal_origin === animal.animal_origin,
  );
  const [targetId, setTargetId] = useState(candidates[0]?.id || "");
  const [primarySide, setPrimarySide] = useState<"current" | "target">(
    "current",
  );
  ...
  const preview = useMemo(() => {
    ...
    return buildMergedAnimal(primary, secondary, {
      nameChoice,
      genderChoice,
      neuterChoice,
      healthChoice,
      ...
      animals: state.animals.map((item) => {
        ...
        if (item.id === secondary.id)
          return {
            ...
            merged_into_animal_id: primary.id,
            updated_at: now,
          };
        return item;
      }),
      ...
      mergeLogs: [
        {
          ...
          imported_location_ids: importedLocations.map(
        ...
        {target && primary && secondary && preview ? (
          <>
            <div className="grid grid-cols-2 gap-3">
              <AnimalMini
                animal={animal}
                active={primarySide === "current"}
                onClick={() => setPrimarySide("current")}
                label="当前档案"
              />
              <AnimalMini
                animal={target}
                active={primarySide === "target"}
                onClick={() => setPrimarySide("target")}
                label="合并对象"
              />
            </div>
            ...
            <MergeField
              label="性别"
              a={genderLabels[primary.gender]}
              b={genderLabels[secondary.gender]}
              choice={genderChoice}
            ...
            <MergeField
              label="绝育状态"
              a={
                primary.neuter_status
                  ? neuterLabels[primary.neuter_status]
                  : "未知"
              }
              b={
                secondary.neuter_status
                  ? neuterLabels[secondary.neuter_status]
                  : "未知"
              }
              choice={neuterChoice}
              onChange={setNeuterChoice}
            />
            <MergeField
              label="健康状态"
              a={
                primary.health_status
                  ? healthLabels[primary.health_status]
                  : "未知"
              }
              b={
                secondary.health_status
                  ? healthLabels[secondary.health_status]
                  : "未知"
              }
              choice={healthChoice}
              onChange={setHealthChoice}
            />
            ...
            <div className="space-y-3 rounded-lg bg-white p-4 ring-1 ring-sand/70">
              ...
              <select
                ...
                onChange={(event) =>
                  setImportLocations(
                    event.target.value as "none" | "private" | "blurred",
                  )
                }
              >
            ...
            <div className="rounded-lg bg-white p-4 ring-1 ring-sand/70">
              ...
              <p className="text-sm text-stone-600">
                别名：
                {preview.aliases?.length ? preview.aliases.join("、") : "无"}
              </p>
            </div>
...
function buildMergedAnimal(
  ...
  const name =
    choices.nameChoice === "secondary" ? secondary.name : primary.name;
  return {
    ...primary,
    name,
    aliases: [...aliases].filter((alias) => alias && alias !== name),
    gender:
      choices.genderChoice === "secondary" ? secondary.gender : primary.gender,
    neuter_status:
      choices.neuterChoice === "secondary"
        ? secondary.neuter_status
        : primary.neuter_status,
    health_status:
      choices.healthChoice === "secondary"
        ? secondary.health_status
        : primary.health_status,
...
function AnimalMini({
  ...
  return (
    <button
      className={`rounded-lg p-3 text-left ring-1 ${active ? "bg-orange-50 ring-clay" : "bg-white ring-sand/70"}`}
      onClick={onClick}
    >
      <img
